    ...
    for GMFile in GMFilesInChronologicalOrder[:-1]:
        ...
        for fkey, ckey in zip(fmovenames2, cmovenames2):
            if not (fkey in fmovenames):
                fmovedata[key] = fmovedata2[key]
                fmovenameslegacy += fmovedata2[key].name
            if not (ckey in cmovenames):
                cmovedata[key] = cmovedata2[key]
...
def print_raidboss_CPs():
    fmovedata, cmovedata, speciesdata, CPMultiplier, typeadvantages = importAllGM()
    pkmns = []
    for raidlvl in range(1,len(raids_list)):
        for bossname in raids_list[raidlvl]:
            dex = getPokedexNumber(bossname, speciesdata)
            fmove = speciesdata[dex].fmoves[0]
            cmove = speciesdata[dex].cmoves[0]
            pkmn = pokemon(speciesdata[dex], [15,15,15], 0, fmove, cmove, poketype="raid_boss")
            print("%12s has %dCP" % (bossname, pkmn.CP))
            pkmns += [pkmn]
